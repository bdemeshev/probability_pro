...
\begin{problem}
Величина $U$ распределена равномерно на отрезке $[0;1]$. 
Рассмотрим две тройки случайных величин, тройку $X = \tan(\pi U/2)$, $Y= \tan(\pi U/2)$ и $Z = - 2\tan(\pi U/2)$, и тройку $X'=\tan(\pi U/2)$, $Y' = \tan(\pi (1-U)/2)$ и $Z'=-2\tan(\pi \abs{2U-1}/2)$.

\begin{enumerate}
  \item Найдите $\E(X + Y + Z)$.
  \item Правда ли что одинаково распределены $X$ и $X'$, $Y$ и $Y'$, $Z$ и $Z'$?
  \item Найдите $\E(X' + Y' + Z')$.
...
\begin{sol}
  $\E(X+Y+Z) = 0$, $\E(X'+Y'+Z') = \frac{4}{\pi}\ln 2$, $X\sim X'$, $Y\sim Y'$, $Z\sim Z'$, парадок возникает из-за бесконечности ожиданий $X$, $Y$ и $Z$.
\end{sol}
\end{problem}
...
У Васи  $n$  знакомых девушек (их всех зовут по-разному). 
Он пишет им  $n$  писем, но, по рассеянности, раскладывает их в конверты наугад. 
Величина  $X$  обозначает количество девушек, получивших письма, написанные лично для них. 
Найдите  $\E(X)$, $\Var(X)$.
\begin{sol}
$\E(X)=1$, $\Var(X)=1$
\end{sol}
