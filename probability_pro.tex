...
Если мы будем заставлять Машу есть конфеты из вазы миллион раз\footnote{говоря языком статистики, посмотрим на реализации независимых случайных величин $X_1, X_2, \ldots, X_{1'000'000}$, распределение которых совпадает с распределением $X$}, иногда она будет съедать одну конфету, иногда две, а иногда три. Но значение среднего\footnote{То есть значение случайной величины $\bar{X}=(X_1 + X_2 + \ldots + X_{1'000'000})/1000000$} будет близко к 1.5\footnote{Оно будет \textit{равняться} 1.5 только при стремлении количества наблюдений к бесконечности}.
\end{sol}
\end{problem}
...
У \textit{абсолютно непрерывной случайной величины} есть \textit{функция плотности},
$f_X(t)$, $\P(X \in [a;b]) = \int_a^b f(t) dt$ или $\P(X \in [x; x+ \Delta]) \doteq f(x) \cdot \Delta$.


Здесь мы популяризируем мощный стиль сэра Исаак Ньютона \autocite{newton1999principia} в современных обозначениях!
...
\begin{enumerate}
 ...
 \item Найдите функции плотности и функции распределения величин $Y=\ln X$.
 \item Найдите функции плотности и функции распределения величин $W=(X-0.5)^2$.
 \item Найдите функции плотности и функции распределения величин $Z=X^2$.
 \end{enumerate}
