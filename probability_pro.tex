...
\begin{enumerate}
    ...
    \item Решается это методом первого шага. В самом начале игры на кону ноль рублей, и если выпадает \textcircled{4}, \textcircled{5} или \textcircled{6}, мы не получаем ничего. 
    Если выпадает \textcircled{1}, \textcircled{2} или \textcircled{3}, то мы как бы начинаем точно такую же игру (с теми же вероятностями и тем же матожиданием выигрыша), но сумма на кону на 1, 2 или 3 рубля больше (но еще не факт, что мы их получим, поэтому домножаем на вероятность забрать выигрыш):
    \[\E(S) = 1/6(\E(S) + \mu) + 1/6(\E(S) + 2 \cdot \mu) + 1/6(\E(S) + 3  \cdot  \mu) + 1/6  \cdot  0 + 1/6  \cdot  0 + 1/6  \cdot  0 = 1\frac{1}{3},\]где $\mu$ -- вероятность забрать накоп раньше, чем он сгорит (2/3).
    ...
    \item Меняется только слагаемое для \textcircled{5} -- игра продолжится (с теми же вероятностями и тем же матожиданием выигрыша):
    \[\E(S) = 1/6(\E(S) + \mu) + 1/6(\E(S) + 2 \cdot \mu) + 1/6(\E(S) + 3  \cdot  \mu) + 1/6  \cdot  0 + 1/6  \cdot  \E(S) + 1/6  \cdot  0 = 2\]
\end{enumerate}

На семинаре рассматривался ещё один вариант -- кладём m рублей в карман (<<несгораемая сумма>>) и игра продолжается ($1/6(\E(S) + m)$)
...
\[
\P(A^с \cap B^с) = \P((A\cup B)^c) = 1-\P(A\cup B) = 1-\P(A)-\P(B) = 1-0.7=0.3
\]
...
\begin{problem}
 ...
 \begin{enumerate}
     ...
     \item Найдите $\E(X)$ из геометрических соображений. Что больше, $\E(X)$ или 1?
...
В общем случае: \[\P(A\cup B\cup C) = \P(A) + \P(B) + \P(C) - \P(A, B) - \P(B, C) - \P(A, C) + \P(A, B, C).\]
Мы знаем, что A и B несовместны, значит, $\P(A, B) = \P(A, B, C) = 0$. 
По формуле условной вероятности $\P(B, C) = \P(B|C)\P(C)$, а $\P(A,C) = \P(A|C)\P(C)$. 
Так как A и C независимы, то $\P(A|C) = \P(A)$. 
Собирая всё это вместе, получаем: \[\P = 0.3 + 0.4 + 0.5 - 0.1 \cdot 0.5 - 0.3 \cdot 0.5 = 1\]
...
Так как о функции плотности нам известно только её значение в точке, лучшим приближением значения в некоторой окрестности точки, которое мы можем дать, будет константа: $\P(\xi \in [7,7.001]) \approx f(7) \cdot 0.001 = 0.005$
...
Получается $F_Y(y) = \begin{cases}
    e^{3y}, \text{ если } y \leq 0, \\
    1, \text{ если } y > 0.
\end{cases}$ и $f_Y(y) = \begin{cases}
...
% lln
\begin{problem}
 Величины $X_1$, \ldots, $X_n$ независимы и равномерно распределены на отрезке $[0;1]$. 
 Найдите $\plim \bar X_n$, $\plim 1/(1 +\bar X_n)$, $\plim \sum_{i=1}^n \ln X_i / n$, $\plim \sqrt[n]{X_1 \cdot \ldots \cdot  X_n}$, $\plim (X_1 \cdot \ldots \cdot X_n)$, $\plim \max\{X_1, \ldots, X_n \}$, $\plim \min \{X_1, \ldots, X_n\}$, $\plim \frac{\sum (X_i - \bar{X})^2}{n}$, $\plim X_1 / \bar{X}$.
...
\begin{problem}
 Величины $X_1$, \ldots, $X_n$ независимы и равномерно распределены на отрезке $[0;1]$. 
 \begin{enumerate}
  \item Найдите примерный закон распределения величин $\bar X_{100}$, $S_{100} = X_1 + \ldots + X_{100}$.
...
\begin{problem}
 ...
 Найдите вероятность того, что через сто дней акция будет стоить больше 1030 рублей.
\begin{sol}
...
Найдите примерно ожидания и дисперсии для величин $R = X/Y$ и $Z = X\cdot Y$.
\begin{sol}
\end{sol}
\end{problem}
...
Найдите примерно закон распределение абсциссы и ординаты вершины\footnote{Это простейшая модель, подходящая, например, для поиска оптимальной дозировки лекарства.}. 
\begin{sol}
\end{sol}
\end{problem}
  
% Здесь надо добавить задачу про логит модель и предельные эффекты!
...
\begin{problem}
 Неправильный кубик\index{кубик} выпадает с вероятностью $0.5$ шестеркой вверх. 
