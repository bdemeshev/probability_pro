...
\begin{sol}
Нарисуем дерево, обозначая в вершинах «стадии» эксперимента, в листьях – итоговый результат, а «ветки» будем подписывать вероятностями выпадения\footnote{В целях краткости и читаемости, подписи «стадий» можно обозначать символически. В нашем случае символическая запись «1х 3о» означает, что в вазе 1 конфета с орехом и 3 -- без ореха}.
...
\begin{sol}
Пусть $N$ -- количество извлечённых лампочек.
Нарисуем дерево
...
а) Требуется найти вероятность $\P(N=3)$. 
Исход $N=3$ может произойти двумя способами.
Следуя по стрелочкам, получим 

\[
P(N=3) = \frac{1}{2}\cdot\frac{2}{3}\cdot\frac{1}{2} + \frac{1}{2}\cdot\frac{2}{3}\cdot\frac{1}{2} = \frac{1}{2}
\]

б) Чтобы найти математическое ожидание, найдём закон распределения $N$. 
...
& \P(N=4) = 1 - \P(N=2) - \P(N=3) = 1 - \frac{1}{6} – \frac{1}{2} = \frac{1}{3}
\end{flalign*}
...
$$
\E(X) = 2\cdot\frac{1}{6} + 3\cdot\frac{1}{2} + 4\cdot\frac{1}{3} = \frac{19}{6}
$$
\end{sol}
\end{problem}
...
\[
\E(X) = 0\cdot\frac{1}{8} + 1\cdot\frac{1}{2} + 2\cdot\frac{3}{8} = \frac{9}{8}
\]
\end{sol}
\end{problem}
...
Тогда математическое ожидание $E(X) = 0\cdot\frac{1}{8} + 1\cdot\frac{3}{8} + 2\cdot\frac{3}{8} = \frac{9}{8}$
...
\end{enumerate}
\begin{sol}
а) Представим, что 30 человек собираются в одной комнате и входят по очереди.
Пусть имя первого -- Aлексей, второго -- Bлад, третьего -- Саша.

В комнату зашёл Aлексей, теперь заходит Bлад. 
Вероятность совпадения их дней рождения равна $1/365\cdot1/365\cdot 365=1/365$\footnote{Потому что вероятность одинаково наугад выбрать 2 одинаковых дня в году равна $1/365\cdot1/365$, а всего дней 365}.
Значит вероятность не совпадения дней рождения равна $364/365$.
Теперь заходит Cаша. 
Дни рождения людей в комнате не совпадают только если они не совпали до появления Cаши, и день рождения Cаши не совпал ни с днём рождения Aлексея, ни с днём рождения Bлада.
Другими словами, вероятность не совпадения равна\footnote{В первом переходе использована теорема Моргана, и чертой обозначается дополнение}

\begin{align*}
    & \frac{364}{365}\cdot\P(\text{Алексей и Саша родились в разные дни И Влад и Саша родились в разные дни)}=\\
    & \frac{364}{365}\cdot\P(\overline{\text{Алексей и Саша родились один день ИЛИ Влад и Саша родились в один день)}}=\\
    & \frac{364}{365}\cdot\left[1 - \P(\text{Алексей и Саша родились один день ИЛИ Влад и Саша родились в один день)}\right]=\\
    & \frac{364}{365}\cdot\left[1 - \P(\text{Алексей и Саша родились один день}) - \P(\text{Влад и Саша родились в один день)}\right]=\\
    & \frac{364}{365}\cdot\left[1 - \frac{1}{365} - \frac{1}{365}\right] = \frac{364}{365}\cdot\frac{363}{365}
\end{align*}

Очевидно, такая логика распространяется на сколь угодно много человек в комнате. 
Для того, чтобы понять, какой множитель будет последним в случае 30 человек, обозначим вероятность не совпадения дней рождения после захода $i$-го человека в комнату за $p_i$.
Тогда

\[
p_1 = \frac{365}{365} = 1, \quad p_2 = \frac{364}{365}, \quad p_3 = \frac{364}{365}\cdot\frac{363}{365}, \quad p_{30} = \text{?}
\]

Заметим, сумма нижнего индекса $p_i$ и числителя последнего множителя всегда равняется $366$, а значит 
...
б) Вероятность хотя бы одного совпадения равна единице за вычетом вероятности не совпадения. 
Значит, искомая вероятность для $n$ человек в комнате равна

\[
\P(\text{хотя бы одно совпадение дней рождения}) = 1 -\frac{364!}{(365-n)!}\cdot\frac{1}{365^{n-1}} = f(n)
\]

Остаётся найти решение неравенства $f(n)>1/2$.
Оказывается, уже при 23 людях неравенство выполняется.
\end{sol}
\end{problem}

\begin{problem}
Саша и Маша по очереди подбрасывают кубик\index{кубик} до первой шестёрки. 
...
Сколько в среднем раз они будут бросать кубик?
...
Пусть $p$ -- вероятность, что Маша будет мыть посуду, если она бросает кубик. 
...
Пусть $\mu$ -- математическое ожидание бросков, если Маша бросает кубик. 
Тогда верно следующее равенство
...
\begin{problem}
 ...
 Если на кубике\index{кубик} выпадает 1, 2 или 3, то соответствующее количество монет добавляется на кон.
...
а) Пусть $a$ -- вероятность выигрыша Маши, если она находится в старте, $b$ -- вероятность выигрыша Маши, если она находится в «О» и так далее, как на рисунке ниже
...
б) Все случаи, когда подбрасываний четыре -- это «abddm», «abces», «acces».
...
\begin{cases}
    ...
    e = \frac{1}{2}\cdot c + \frac{1}{2}\cdot 1
\end{cases}
\]
...
\begin{problem}
Вася подкидывает кубик\index{кубик} до тех пор, пока на кубике не выпадет единица, или пока он сам не скажет «Стоп». 
...
\begin{align*}
    \P(\text{разного}) & = \P(\text{первый красный, второй чёрный}) + \P(\text{первый чёрный, второй красный})=\\
    & = \frac{1}{2} \cdot \frac{1}{1+2} + \frac{1}{2} \cdot \frac{1}{2+1} = \frac{2}{3}
\end{align*}


Сумма в знаменателе -- количество оставшихся тузов. 
После выбора одной карты останется один туз такого же цвета и два -- противоположного.

\end{sol}
\end{problem}
\begin{problem}
 События $A$  и  $B$ несовместны, то есть не могут произойти одновременно. Известны вероятности $\P(A)=0.3$, $\P(B)=0.4$. 
 Найдите\footnote{Событие $A^c$ — это событие противоположное событию $A$, иногда обозначается $\bar{A}$} $\P(A^{c} \cap B^{c} )$.

\begin{sol}

Несовместные события -- такие, что $\P(A\cup B) = \P(A) + \P(B)$ или $\P(A\cap B) = \varnothing$. 

По теореме Моргана 

\[
\P(A^с \cap B^с) = \P(\overline{A\cup B}) = 1-\P(A\cup B) = 1-\P(A)-\P(B) = 1-0.7=0.3
\]
...
Нарисуем дерево, вероятности перехода из стадии в стадию опустим, они равны $1/3$
...
\Loop[dist=3cm, style={very thick}, label={$t + 1$}, labelstyle={fill=white}, dir=WE,style={->}](s)
\end{tikzpicture}
\end{center}
...
\[
\mu = \frac{1}{3}\cdot1 + \frac{1}{3}\cdot(\mu + 1) + \frac{1}{3}\cdot(\mu + 2), \quad\mu = 4 
\]
...
\begin{problem}
 Треугольник с вершинами $(0;0)$, $(2;0)$ и $(1;1)$. 
...
\begin{problem}
 Треугольник с вершинами $(0;0)$, $(2;0)$ и $(2;1)$. 
 Внутри него случайным образом выбирается точка, $X$ — абсцисса точки. 
 
 Найдите $\P(X>1)$, $\P(X\in [0.5;1])$. Что больше, $\E(X)$ или 1?
 Найдите $\E(X)$ из геометрических соображений.

\begin{sol}
  Вертикальная прямая $x = \E(X)$ должна уравновешивать металлическую треугольную пластинку. 
...
\begin{problem}
Исследовательница Мишель подкидывает игральный кубик\index{кубик} неограниченное количество раз и складывает выпадающие количества очков.
\begin{enumerate}
...
\begin{problem}
Упрямая исследовательница Мишель подбрасывает монетку до тех пор, пока количество орлов не окажется в точности равным удвоенному количеству решек. 
 Монетка выпадает орлом с вероятностью $p$. Какова вероятность того, что Мишель будет подкидывать монетку вечно?
...
\begin{problem}
Исследовательница Мишель хочет встать утром с правой ноги с вероятностью $1/\sqrt{2}$, и с левой с вероятностью $1 - 1/\sqrt{2}$. 
Однако для проведения случайных экспериментов у неё есть только одна правильная монетка. 
Как с помощью правильной монетки ей добиться цели?
...
выбирает равномерно 10 точек на отрезке $[0;30]$.
...
Да, предел функции плотности может быть отличен от нуля, она может подниматься, скажем до значения равного одному, на отрезках убывающей длины. 
\end{sol}
...
Как справедливо заметил исследователь Винни-Пух\index{герои!Винни-Пух}, всякая вещь — или есть, или нет. 
...
\begin{problem}
Винни-Пух\index{герои!Винни-Пух} и Пятачок\index{герои!Пятачок}  играют в Пустяки (Poohsticks). 
Каждую партию Винни-Пух\index{герои!Винни-Пух}  выигрывает с вероятностью $p=0.7$. 
Всего они сыграли $n=10$ партий. Пусть $X$ — количество партий, выигранных Винни-Пухом\index{герои!Винни-Пух} .
\begin{enumerate}
...
Храбрый Пятачок\index{герои!Пятачок}  спешит с ружьём на помощь зависшему исследователю Винни-Пуху\index{герои!Винни-Пух} . 
...
\begin{enumerate}
  ...
  \item Терпения Винни-Пуха\index{герои!Винни-Пух}  хватает на 5 промахов Пятачка. Какова вероятность того, что терпения Винни-Пуха не хватит?
  ...
  \item Чему будут равны $\P(X=k)$, $\E(X)$ и $\Var(X)$, если при отдельном выстреле Пятачок\index{герои!Пятачок}  попадает с вероятностью $p$?
...
Торопливый Пятачок\index{герои!Пятачок}  снова стреляет в шарик. 
При каждом выстреле торопливый Пятачок попадает с вероятностью $p$. 
Пятачок\index{герои!Пятачок} делает $d$ выстрелов в минуту. Для данной задачи будем считать, что после попадания Пятачка в шарик, шарик мгновенно заменяется на новый.  Пусть $X$ — это номер выстрела первого попадания, $Z$ — время первого попадания, а $Y$ — количество попаданий за первую минуту.
\begin{enumerate}
...
Торопливый Пятачок\index{герои!Пятачок} очень торопится, поэтому величина $d$ крайне велика. Однако когда Пятачок\index{герои!Пятачок} торопится, он чаще промахивается, и оказывается, что при любом $d$ среднее количество попаданий в минуту постоянно и равно $\lambda=10$.
...
\begin{enumerate}[resume]
  ...
  \item Какова при большом $d$ вероятность того, что Пятачок\index{герои!Пятачок} впервые попадёт в первые 5 минут? Впервые попадёт с 10-ой по 15-ую минуты, если известно, что в первые 10 минут попаданий не было?
...
Вовочка\index{герои!Вовочка} получает пятерку с вероятностью 0.1, четверку — с вероятностью 0.2, тройку — с вероятностью — 0.3 и двойку с вероятностью 0.4. 
В этом четверти он писал 20 контрольных. Какова вероятность того, что все оценки у Вовочки одинаковые? Сколько разных оценок он в среднем получит?

\begin{sol}
\end{sol}
\end{problem}
\begin{problem}
 «Судьба Дон Жуана\index{герои!Дон Жуан}»
...
\begin{problem}
 ...
 однако $X$ и $Y$ равномерно распределены на отрезке $[0;1]$.
\begin{sol}
...
\begin{problem}
Величина  $X$  равномерно распределена на $[-1;1]$ и $Y=X^{2}$.
\begin{enumerate}
\item Найдите  $\Cov(X,Y)$;
\item Верно ли, что $X$ и $Y$ независимы?
\end{enumerate}
...
\begin{problem}
 ...
 Случайным образом, равномерно на этом кольце, выбирается точка, $X$ и $Y$ — её координаты.
...
\begin{problem}
В Пуассоновском потоке с интенсивностью $\lambda$ время наступления $k$-го события распределено согласно гамма-распределению $Gamma(k, \lambda)$ 
...
\begin{problem}
Случайным образом на сфере $x^2+y^2+z^2=1$ равномерно выбирается точка. 
...
\end{enumerate}
\begin{sol}
Величина $X$ равномерна на $[-1;1]$; пара $(X, Y)$ равномерна в круге $x^2+y^2 \leq 1$.
\end{sol}
\end{problem}
...
\begin{problem}
  Вася хочет посчитать сумму 100 случайных слагаемых. Слагаемые независимы и равномерны распределены
...
\begin{problem}
  ...
  Их очень много. Количество зайев на каждом пеньке — случайная величина равновероятно принимающая значения от 1 до 19. 
  У Деда Мазая две стратегии. 
  
  Стратегия А: перевозить зайецев на безопасное место равными партиями по 10 штук. 
  
  Стратегия Б: спасать за одну перевозку зайцев с одного пенька. 
...
Какое распределение примерно имеют величины $\ln(X/n)$? $X/(n-X)$?
\begin{sol}
\end{sol}
\end{problem}
...
\begin{problem}
Величины $X$ и $Y$ имеют \textit{небольшую} дисперсию. 

Найдите примерно ожидания и дисперсии для $R = X/Y$ и $Z = X\cdot Y$.
\begin{sol}
\end{sol}
\end{problem}
...
\begin{problem}
 Неправильный кубик\index{кубик} выпадает с вероятностью $0{,}5$ шестеркой вверх. Остальные пять граней выпадают равновероятно. Случайная величина $X$ — остаток от деления номера грани на два, $Y$ — остаток от деления номера грани на три. Найдите
\begin{enumerate}
...
\begin{sol}
  ...
  У равномерной на $[0;a]$ спутанность равна $a$.
\end{sol}
\end{problem}
...
\begin{enumerate}
  \item Вектор $(X, Y)'$ равномерно распределён на множестве $\{x^2 \leq 1, \; y^2 \leq 1\}$.
  \item Вектор $(X, Y)'$ равномерно распределён на множестве $\{x^2 + y^2 \leq 1\}$.
...
\item Если Вовочка\index{герои!Вовочка} плохо пишет контрольную, то его лишают мороженого. 
После этого успеваемость Вовочки\index{герои!Вовочка} как правило улучшается. В чём сходство этой ситуации с данной задачей?
...
\begin{problem}
 ...
 Храбрая исследовательница Мишель подкидывает правильную монетку. 
 Если монетка выпадает орлом, то Мишель домножает величину $X$ на единицу, а иначе — на минус единицу, и получает величину $Y$.
\begin{enumerate}
